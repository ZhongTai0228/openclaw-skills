...
- 对齐：`text-align: justify`（写在 `<p>` 上，**不能写在 `<span>` 上**，否则无效）
- **不要加** `word-break: break-all` 或 `overflow-wrap: break-word`——默认断词行为就是最佳效果：英文单词整体换行，上一行中文自动两端对齐
...
## 尾部图片（最后三张）
- 段后距缩小：`margin-bottom: 8px`（而非常规的 24px）
- 紧凑排列，减少底部留白
